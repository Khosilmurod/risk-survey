...
def create_new_trials_matching_style():
    ...
    print(f"Created new.csv with {len(new_trials)} trials matching full_trials.csv style")
    
    # Show preview of the first few groups
    print("\nFirst 16 rows showing the grouping style:")
    for i, trial in enumerate(new_trials[:16]):
        print(f"Row {i+1}: Trial {trial['trial_id']}, Combo {trial['combination_id']} - {trial['size_condition']} - {trial['risk_probability']}% × {trial['risk_reward']:,} vs {trial['safe_reward']:,}")
    
    print(f"\nTotal combinations: {combination_id - 1}")
    print(f"Total trials: {len(new_trials)}")

if __name__ == "__main__":
    create_new_trials_matching_style()
